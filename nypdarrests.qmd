...
### Arrests and Indicators of Crime Across Precincts
...
The graph presents the racial distribution of arrests across the five boroughs as proportions. On the x-axis, the names of the boroughs are labeled by their abbreviations, with Brooklyn represented by "K." After conducting a chi-sq test of independence, the p-value was \< 0.05, indicating statistical significance. There is statistically significant evidence that racial distribution of arrests is not independent of borough. In other words, racial distribution of arrests differs across the five boroughs.

```{r}
race_boro <- table(arrests$perp_race, arrests$arrest_boro)

chisq.test(race_boro)
```
...
ggplot(race_p75, aes(y = perp_race, x = prop)) +
...
ggplot(race_p22, aes(y = perp_race, x = prop)) +
  geom_col(fill = "darkblue") +
  theme_minimal() +
